...
from .utils import *
from model.config import Elastic



class Elk:
    es = Elasticsearch(Elastic.HOST+Elastic.PORT, timeout=180, max_retries=10, retry_on_timeout=True)
            ...
            for count in range(len(total_vec[v_idx])):
                docs.append({
                    ...
                    '_source': {
                        ...
                        #"yolo_vector_cat": encode_array(total_vec[v_idx][count]['yolo_vector_cat']),
                        #"resnet_vector": encode_array(total_vec[v_idx][count]['resnet_vector']),
                        #"cgd62_vector": encode_array(total_vec[v_idx][count]['cgd62_vector']),
                        #"cgd48_vector": encode_array(total_vec[v_idx][count]['cgd48_vector']),
                        #"cgd10_vector": encode_array(total_vec[v_idx][count]['cgd10_vector']),
                        #"cgd44_vector": encode_array(total_vec[v_idx][count]['cgd44_vector']),
                        #"cgd90_vector": encode_array(total_vec[v_idx][count]['cgd90_vector']),
                        "cgd7_vector": encode_array(total_vec[v_idx][count]['cgd7_vector']),
    ...
    def search_vec(self, search_index, search_vec, size=10):
        ...
        res = self.es.search(
            ...
            body={
                "_source": {
                    "includes": ["_index", "id", "_score", "box_statue", "cat_key", "gs_bucket", "click_url", "image_path", "raw_box"]
                },
    ...
    def multi_search_vec(self, idx_list, vb_list, k, vector_type=None):
        st_time = time.time()
        total_body=''
        for num in range(len(idx_list)):
            index = '{"index":"'+str(idx_list[num])+'"}'
            body_data = '{"_source": {"includes": ["_index", "id", "_score", "box_statue", "cat_key", "gs_bucket", "click_url", "image_path", "raw_box"]},' \
                        '"query": {' \
                        '"function_score": {' \
        ...
        for count in range(len(total_vec)):
            docs.append({
                ...
                '_source': {
                    ...
                    "cgd62_vector": encode_array(total_vec[count]['cgd62_vector']),
                    "cgd48_vector": encode_array(total_vec[count]['cgd48_vector']),
